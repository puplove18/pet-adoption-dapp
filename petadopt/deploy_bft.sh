...
# Step 2: Bring up Fabric network in BFT mode
# this uses test-network BFT config and multiple orderer nodes
banner "Step 2/6 — Starting Fabric network + channel (BFT)"
cd "$TEST_NETWORK"
./network.sh up createChannel -bft -ca -s couchdb

ORDERER_COUNT=$(docker ps --format '{{.Names}}' | grep '^orderer' | wc -l | tr -d ' ')
if [ "${ORDERER_COUNT}" -lt 4 ]; then
    fail "BFT startup check failed — expected at least 4 orderer nodes, found ${ORDERER_COUNT}"
fi

info "BFT network is UP  |  Channel: mychannel  |  Orderers: ${ORDERER_COUNT}"
